...
impl Scene {
    pub fn new(	) -> Self {
        ...
        Scene {objects, lights, light_objects, show_emissive_surfaces: false}
    }
...
/// Computes the light intensity, color by color, of an intersection
    pub fn compute_intensity(&self, ray: Ray, intersection: Intersection, nb_iter_max: usize, time: f64) -> Vector {
        ...
        match nb_iter_max {
            ...
            _ => {
                ...
                cur_intensity += self.compute_point_light(intersection, nb_iter_max, time).max(Vector::new_eq(0.));
                cur_intensity += self.compute_mirror(ray, intersection, nb_iter_max, time).max(Vector::new_eq(0.));
                cur_intensity += self.compute_transparent(ray, intersection, nb_iter_max, time).max(Vector::new_eq(0.));
                cur_intensity += self.compute_emissive( intersection, self.show_emissive_surfaces, time).max(Vector::new_eq(0.));
                cur_intensity += self.compute_indirect(ray, intersection, nb_iter_max, time).max(Vector::new_eq(0.));
                cur_intensity += self.compute_direct(ray, intersection, nb_iter_max, time).max(Vector::new_eq(0.));
...
/// Computes the mirror component of the light intensity, color by color, of an intersection
    pub fn compute_mirror(&self, ray: Ray, intersection: Intersection, nb_iter_max: usize, time: f64) -> Vector {
        match intersection.material.mirror {
            ...
            true => {
                ...
                if let Some(inter) = reflected_inter {
                    self.compute_intensity(reflected_ray, inter.get_inter_nudged(), nb_iter_max - 1, time) * intersection.material.specular_color
                } else {
                    Vector::new_eq(0.)
                }
            }
...
/// Computes the transparency component of the light intensity, color by color, of an intersection
    pub fn compute_transparent(&self, ray: Ray, intersection: Intersection, nb_iter_max: usize, time: f64) -> Vector {
            ...
            true => {
                ...
                match refracted_ray {
                    ...
                    Some(refracted_ray_a) => {
                        ...
                        let refracted_intersection = self.compute_intersection(refracted_ray_a, time);
...
/// Computes the indirect lightning component of the light intensity, color by color, of an intersection
    pub fn compute_indirect(&self, ray: Ray, intersection: Intersection, nb_iter_max: usize, time: f64) -> Vector {
        let mut cur_intensity = Vector::new(0., 0., 0.);
        ...
        match intersection.material.phong && rand >= p {
            ...
            true => {
                ...
                let reflected_ray = ray.reflect(intersection);
                new_ray = Ray::new_rand_ray_phong(intersection.get_point_nudged(), intersection.material.phong_exponent, reflected_ray.direction );
        ...
        if let Some(inter) = new_intersection {
            ...
            let proba_phong = (intersection.material.phong_exponent + 1.) * new_ray.direction.dot(reflected_ray.direction).powf(intersection.material.phong_exponent);
            ...
            match intersection.material.phong && rand >= p {
                false => {
                    cur_intensity += indirect_intensity * proba_diffuse * intersection.material.color / (2. * PI) / proba;
                },
                true => {
                    cur_intensity += indirect_intensity * proba_diffuse * (intersection.material.phong_exponent + 2.) * new_ray.direction.dot(reflected_ray.direction).powf(intersection.material.phong_exponent) * intersection.material.specular_color / (2. * PI) / proba;
                }
            }    
...
/// Computes the emissive surface component of the light intensity, color by color, of an intersection
	pub fn compute_emissive(&self, intersection: Intersection, show_emissive_surfaces: bool, _time: f64) -> Vector {

        match intersection.material.emissive && show_emissive_surfaces {
            false => {Vector::new_eq(0.) },
            true => {Vector::new_eq(1.) * intersection.material.color * intersection.material.emissivity}
            }
        }
    
/// Computes the direct lightning component of the light intensity, color by color, of an intersection
	pub fn compute_direct(&self, ray: Ray, intersection: Intersection, _nb_iter_max: usize, time: f64) -> Vector {
        let mut cur_intensity = Vector::new(0., 0., 0.);
        ...
        for i in 0..self.light_objects.len() {
            probas.push(self.light_objects[i].get_material().emissivity / self.light_objects[i].get_surface_area());
        }
        ...
        for i in 0..self.light_objects.len() {
        {
            if rand*sum <= probas[i]
            {
                ...
                new_ray = Ray::new_rand_ray_angle_uniform(light_center, light_object_i.get_surface_area(), dir_center_light);

                let rand_result_point = new_ray.origin;
                let rand_result_dir = new_ray.direction.normalize();
                let rand_result_dir_to_intersection = (intersection.point - rand_result_point).normalize();
                let d = (intersection.point - rand_result_point).norm_sq();
                let nudged = intersection.get_point_nudged();

                let mut new_light = Light::new(rand_result_point, Vector::new_eq(1.) * light_object_i.get_material().emissivity / light_object_i.get_surface_area() * light_object_i.get_material().color);

                for anim in light_object_i.get_animations() {
                    new_light.add_animation(anim);
                }
                
                if self.compute_shadows(nudged, &new_light, time) == true
                {
                    if intersection.material.phong == true
                    {
                        let reflected_ray = ray.reflect(intersection);

                        let phong_term = reflected_ray.direction.dot(rand_result_dir_to_intersection).powf(intersection.material.phong_exponent)*(intersection.material.phong_exponent + 2.) / (2.*PI);
                        
                        let color_phonged = intersection.material.color + intersection.material.specular_color * (phong_term - 1.);
                        
                        cur_intensity += Vector::new_eq(1.) * light_object_i.get_material().emissivity * light_object_i.get_material().color * intersection.normal.dot(rand_result_dir_to_intersection * (-1.)).max(0.)
                            * light_object_i.get_surface_area() * rand_result_dir.dot(rand_result_dir_to_intersection).max(0.)
                             * color_phonged / (dir_center_light.dot(rand_result_dir).max(0.) * d * 4. * PI);
                    }
                    else
                    {
                        cur_intensity += Vector::new_eq(1.) * light_object_i.get_material().emissivity * light_object_i.get_material().color * intersection.normal.dot(rand_result_dir_to_intersection * (-1.)).max(0.)
                            * light_object_i.get_surface_area() * rand_result_dir.dot(rand_result_dir_to_intersection).max(0.)
                            * intersection.material.color / (dir_center_light.dot(rand_result_dir).max(0.) * d * 4. * PI);
...
/// Computes the point light component of the light intensity, color by color, of an intersection
    pub fn compute_point_light(&self, intersection: Intersection, _nb_iter_max: usize, time: f64) -> Vector {
        let mut cur_intensity = Vector::new(0., 0., 0.);
        for light in self.lights.iter() {
            if self.compute_shadows(intersection.get_point_nudged(), &light, time) == true {
